...
#[inline]
pub(in crate::measure) fn parse_decimal_text_exact(value: &str) -> Result<Decimal, MeasurementError> {
    let (base, scientific_exponent) = split_scientific_text(value).ok_or(MeasurementError::InvalidMeasurementSyntax)?;
    let (mantissa, trailing_zeroes, fraction_digits, negative) = parse_decimal_base(base)?;
    let raw_exponent = scientific_exponent.saturating_sub(i64::try_from(fraction_digits).unwrap_or(i64::MAX));
    let normalized_exponent = raw_exponent.saturating_add(i64::try_from(trailing_zeroes).unwrap_or(i64::MAX));
    let preferred_scale = negative_exponent_scale(raw_exponent);

    crate::__private::finalize_exact_decimal(mantissa, normalized_exponent, preferred_scale, negative)
...
fn split_scientific_text(value: &str) -> Option<(&str, i64)> {
    ...
    match exponent_index {
        Some(index) => Some((&value[..index], parse_scientific_exponent(&value[index + 1..])?)),
        None => Some((value, 0)),
    }
}
...
fn parse_scientific_exponent(value: &str) -> Option<i64> {
    ...
    for digit in digits {
        exponent = exponent.saturating_mul(10).saturating_add(i64::from(digit - b'0'));
    }
    Some(if negative { exponent.saturating_neg() } else { exponent })
}
    ...
    D: Deserializer<'de>,
{
    ...
    if value.len() > MeasurementParseOptions::DEFAULT_MAX_TEXT_BYTES {
        return Err(D::Error::custom(MeasurementError::MeasurementTextLimitExceeded {
