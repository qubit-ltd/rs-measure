...
impl From<Duration> for Measurement<Time> {
    ...
    fn from(duration: Duration) -> Self {
        let total_nanos =
            u128::from(duration.as_secs()) * NANOS_PER_SECOND + u128::from(duration.subsec_nanos());
        let mantissa =
            i128::try_from(total_nanos).expect("Duration total nanoseconds always fit i128");
...
impl TryFrom<Measurement<Time>> for Duration {
    ...
    fn try_from(measurement: Measurement<Time>) -> Result<Self, Self::Error> {
        ...
        let total_nanos = u128::try_from(nanoseconds.to_integer()).map_err(|_| {
        ...
        let maximum_nanos =
            u128::from(u64::MAX) * NANOS_PER_SECOND + u128::from(Duration::MAX.subsec_nanos());
