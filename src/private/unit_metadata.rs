...
pub const fn assert_unit_family_metadata(quantity: &str, symbols: &[&str], aliases: &[&str]) {
    ...
    while index < symbols.len() {
        ...
        assert!(!symbol.is_empty(), "canonical unit symbol must not be empty",);
        assert!(
        ...
        while other < symbols.len() {
            assert!(!str_eq(symbol, symbols[other]), "canonical unit symbols must be unique",);
            other += 1;
        }
        index += 1;
    }
    ...
    while index < aliases.len() {
        ...
        while other < aliases.len() {
            assert!(!str_eq(alias, aliases[other]), "unit aliases must be unique",);
            other += 1;
        }
...
const fn has_leading_unit_whitespace(value: &str) -> bool {
    ...
    if bytes.len() >= 3 {
        return (bytes[0] == 0xE1 && bytes[1] == 0x9A && bytes[2] == 0x80)
            || (bytes[0] == 0xE2 && bytes[1] == 0x80 && matches!(bytes[2], 0x80..=0x8A | 0xA8 | 0xA9 | 0xAF))
...
const fn has_trailing_unit_whitespace(value: &str) -> bool {
    ...
    if length >= 3 {
        return (bytes[length - 3] == 0xE1 && bytes[length - 2] == 0x9A && bytes[length - 1] == 0x80)
            || (bytes[length - 3] == 0xE2
                && bytes[length - 2] == 0x80
                && matches!(bytes[length - 1], 0x80..=0x8A | 0xA8 | 0xA9 | 0xAF))
            || (bytes[length - 3] == 0xE2 && bytes[length - 2] == 0x81 && bytes[length - 1] == 0x9F)
            || (bytes[length - 3] == 0xE3 && bytes[length - 2] == 0x80 && bytes[length - 1] == 0x80);
    }
    false
}
