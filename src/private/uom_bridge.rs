...
#[inline]
pub fn base_f64_to_unit_value(base_value: f64, definition: UnitDefinition) -> Result<Decimal, crate::MeasurementError> {
    let factor = definition.factor();
    let numerator = decimal_to_f64_approx(factor.numerator());
    let denominator = decimal_to_f64_approx(factor.denominator());
    let offset = decimal_to_f64_approx(definition.offset());
...
pub fn decimal_from_f64_approx(value: f64) -> Result<Decimal, crate::MeasurementError> {
    Decimal::from_f64(value).ok_or_else(|| crate::MeasurementError::DecimalConversion(value.to_string()))
}
