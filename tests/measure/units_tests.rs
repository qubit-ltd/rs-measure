...
fn test_length_unit_all_lists_supported_units() {
    let symbols: Vec<&str> = unit::Length::all()
        .iter()
    ...
    assert_eq!(
        symbols,
        vec!["nm", "µm", "mm", "cm", "m", "km", "in", "ft", "yd", "mi"]
    );
}

#[test]
fn test_area_and_volume_unit_all_lists_supported_units() {
    let area_symbols: Vec<&str> =
        unit::Area::all().iter().map(|unit| unit.symbol()).collect();
    let volume_symbols: Vec<&str> = unit::Volume::all()
        .iter()
        .map(|unit| unit.symbol())
        .collect();
...
fn test_mass_and_time_unit_all_lists_supported_units() {
    let mass_symbols: Vec<&str> =
        unit::Mass::all().iter().map(|unit| unit.symbol()).collect();
    let time_symbols: Vec<&str> =
        unit::Time::all().iter().map(|unit| unit.symbol()).collect();

    assert_eq!(
        mass_symbols,
        vec![
            "µg", "mg", "g", "kg", "t", "ct", "oz", "lb", "2000 lb", "2240 lb"
        ],
    );
    assert_eq!(
        time_symbols,
...
fn test_production_quantity_family_units_are_available() {
    let pressure_symbols: Vec<&str> = unit::Pressure::all()
        .iter()
        .map(|unit| unit.symbol())
        .collect();
    let energy_symbols: Vec<&str> = unit::Energy::all()
        .iter()
        .map(|unit| unit.symbol())
        .collect();
    let power_symbols: Vec<&str> = unit::Power::all()
        .iter()
        .map(|unit| unit.symbol())
        .collect();
    let velocity_symbols: Vec<&str> = unit::Velocity::all()
        .iter()
        .map(|unit| unit.symbol())
        .collect();
    let frequency_symbols: Vec<&str> = unit::Frequency::all()
        .iter()
        .map(|unit| unit.symbol())
        .collect();
    let density_symbols: Vec<&str> = unit::MassDensity::all()
        .iter()
        .map(|unit| unit.symbol())
        .collect();
    let temperature_symbols: Vec<&str> = unit::Temperature::all()
        .iter()
        .map(|unit| unit.symbol())
        .collect();
    ...
    assert_eq!(
        pressure_symbols,
        vec![
            "nPa", "µPa", "mPa", "Pa", "hPa", "kPa", "MPa", "bar", "mbar",
            "atm", "mm Hg", "psi"
        ],
    );
    assert_eq!(
...
fn test_electrical_quantity_family_units_are_available() {
    let current_symbols: Vec<&str> = unit::ElectricCurrent::all()
        .iter()
        .map(|unit| unit.symbol())
        .collect();
    let potential_symbols: Vec<&str> = unit::ElectricPotential::all()
        ...
        .map(|unit| unit.symbol())
        .collect();
    let charge_symbols: Vec<&str> = unit::ElectricCharge::all()
        .iter()
        .map(|unit| unit.symbol())
        .collect();
    let capacitance_symbols: Vec<&str> = unit::Capacitance::all()
        .iter()
        .map(|unit| unit.symbol())
        .collect();
    ...
    let conductance_symbols: Vec<&str> = unit::ElectricalConductance::all()
        ...
        .map(|unit| unit.symbol())
        .collect();
    let inductance_symbols: Vec<&str> = unit::Inductance::all()
        .iter()
        .map(|unit| unit.symbol())
        .collect();
...
fn test_mechanical_and_process_quantity_family_units_are_available() {
    let force_symbols: Vec<&str> = unit::Force::all()
        .iter()
        .map(|unit| unit.symbol())
        .collect();
    let acceleration_symbols: Vec<&str> = unit::Acceleration::all()
        .iter()
        .map(|unit| unit.symbol())
        .collect();
    let torque_symbols: Vec<&str> = unit::Torque::all()
        .iter()
        .map(|unit| unit.symbol())
        .collect();
    let angle_symbols: Vec<&str> = unit::Angle::all()
        .iter()
        .map(|unit| unit.symbol())
        .collect();
    let angular_velocity_symbols: Vec<&str> = unit::AngularVelocity::all()
        .iter()
        .map(|unit| unit.symbol())
        .collect();
    let volume_rate_symbols: Vec<&str> = unit::VolumeRate::all()
        .iter()
        .map(|unit| unit.symbol())
        .collect();
    let mass_rate_symbols: Vec<&str> = unit::MassRate::all()
        .iter()
        .map(|unit| unit.symbol())
        .collect();
    let dynamic_viscosity_symbols: Vec<&str> = unit::DynamicViscosity::all()
        .iter()
        .map(|unit| unit.symbol())
        .collect();
    let kinematic_viscosity_symbols: Vec<&str> =
        unit::KinematicViscosity::all()
            .iter()
            .map(|unit| unit.symbol())
            .collect();
    ...
    assert_eq!(
        mass_rate_symbols,
        vec!["mg/s", "g/s", "kg/s", "kg/h", "t/h", "lb/h"]
    );
    assert_eq!(
...
fn test_chemical_medical_and_biological_quantity_family_units_are_available() {
    let amount_symbols: Vec<&str> = unit::AmountOfSubstance::all()
        ...
        .map(|unit| unit.symbol())
        .collect();
    let molar_concentration_symbols: Vec<&str> =
        unit::MolarConcentration::all()
            .iter()
            .map(|unit| unit.symbol())
            .collect();
    ...
    let catalytic_activity_symbols: Vec<&str> = unit::CatalyticActivity::all()
        ...
        .map(|unit| unit.symbol())
        .collect();
    let radioactivity_symbols: Vec<&str> = unit::Radioactivity::all()
        .iter()
        .map(|unit| unit.symbol())
        .collect();
    ...
    assert_eq!(
        amount_symbols,
        vec!["µmol", "mmol", "mol", "kmol", "particle"]
    );
    assert_eq!(
    ...
    assert_eq!(
        catalytic_activity_symbols,
        vec!["µkat", "mkat", "kat", "U", "mU"]
    );
    assert_eq!(
...
fn test_electromagnetic_thermal_optical_and_chemical_extension_units_are_available()
 {
    let electric_field_symbols: Vec<&str> = unit::ElectricField::all()
        .iter()
        .map(|unit| unit.symbol())
        .collect();
    let current_density_symbols: Vec<&str> =
        unit::ElectricCurrentDensity::all()
            .iter()
            .map(|unit| unit.symbol())
            .collect();
    ...
    let resistivity_symbols: Vec<&str> = unit::ElectricalResistivity::all()
        ...
        .map(|unit| unit.symbol())
        .collect();
    let magnetic_flux_density_symbols: Vec<&str> =
        unit::MagneticFluxDensity::all()
            .iter()
            .map(|unit| unit.symbol())
            .collect();
    let magnetic_flux_symbols: Vec<&str> = unit::MagneticFlux::all()
        .iter()
        .map(|unit| unit.symbol())
        .collect();
    let magnetic_field_strength_symbols: Vec<&str> =
        unit::MagneticFieldStrength::all()
            .iter()
            .map(|unit| unit.symbol())
            .collect();
    let heat_capacity_symbols: Vec<&str> = unit::HeatCapacity::all()
        .iter()
        .map(|unit| unit.symbol())
        .collect();
    let specific_heat_capacity_symbols: Vec<&str> =
        unit::SpecificHeatCapacity::all()
            .iter()
            .map(|unit| unit.symbol())
            .collect();
    let thermal_conductivity_symbols: Vec<&str> =
        unit::ThermalConductivity::all()
            .iter()
            .map(|unit| unit.symbol())
            .collect();
    let thermal_resistance_symbols: Vec<&str> = unit::ThermalResistance::all()
        ...
        .map(|unit| unit.symbol())
        .collect();
    let heat_flux_density_symbols: Vec<&str> = unit::HeatFluxDensity::all()
        .iter()
        .map(|unit| unit.symbol())
        .collect();
    let surface_tension_symbols: Vec<&str> = unit::SurfaceTension::all()
        .iter()
        .map(|unit| unit.symbol())
        .collect();
    let luminous_intensity_symbols: Vec<&str> = unit::LuminousIntensity::all()
        ...
        .map(|unit| unit.symbol())
        .collect();
    let illuminance_symbols: Vec<&str> = unit::Illuminance::all()
        .iter()
        .map(|unit| unit.symbol())
        .collect();
    let luminance_symbols: Vec<&str> = unit::Luminance::all()
        .iter()
        .map(|unit| unit.symbol())
        .collect();
    let solid_angle_symbols: Vec<&str> = unit::SolidAngle::all()
        .iter()
        .map(|unit| unit.symbol())
        .collect();
    let molality_symbols: Vec<&str> = unit::Molality::all()
        .iter()
        .map(|unit| unit.symbol())
        .collect();
    let molar_mass_symbols: Vec<&str> = unit::MolarMass::all()
        .iter()
        .map(|unit| unit.symbol())
        .collect();
    let molar_volume_symbols: Vec<&str> = unit::MolarVolume::all()
        .iter()
        .map(|unit| unit.symbol())
        .collect();
    let catalytic_activity_concentration_symbols: Vec<&str> =
        unit::CatalyticActivityConcentration::all()
            .iter()
            .map(|unit| unit.symbol())
            .collect();
    let specific_radioactivity_symbols: Vec<&str> =
        unit::SpecificRadioactivity::all()
            .iter()
            .map(|unit| unit.symbol())
            .collect();
    ...
    assert_eq!(
        resistivity_symbols,
        vec!["mΩ · m", "Ω · m", "Ω · cm", "Ω · mm²/m"]
    );
    assert_eq!(
        magnetic_flux_density_symbols,
        vec!["nT", "µT", "mT", "T", "G"]
    );
    ...
    assert_eq!(
        heat_flux_density_symbols,
        vec!["mW/m²", "W/m²", "kW/m²", "W/cm²"]
    );
    assert_eq!(
        surface_tension_symbols,
        vec!["mN/m", "N/m", "dyn/cm", "J/m²"]
    );
    assert_eq!(luminous_intensity_symbols, vec!["mcd", "cd", "kcd"]);
    assert_eq!(illuminance_symbols, vec!["lx", "klx", "fc"]);
    assert_eq!(
        luminance_symbols,
        vec!["cd/m²", "cd/cm²", "cd/ft²", "fl", "sb"]
    );
    ...
    assert_eq!(
        catalytic_activity_concentration_symbols,
        vec!["kat/m³", "U/L", "mU/mL"]
    );
    assert_eq!(
        specific_radioactivity_symbols,
        vec!["Bq/kg", "Ci/kg", "dpm/kg"]
    );
}
...
fn test_unit_symbols_parse_display_and_serde_round_trip() {
    ...
    assert_unit_symbols_parse_display_and_serde_round_trip::<
